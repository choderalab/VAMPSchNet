...
class VAMPSchNet(Module):
    ...
    def forward(self, data, batch=None):
        ...
        for interaction in self.interactions: # JG: success until this step
...
def train_with_sampler(n_epochs, batch_size=2048, learning_rate=1e-4):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_0, batch_t in loader:
            ...
            data_0 = (z, batch_0)
            data_t = (z, batch_t)
            vschnet.partial_fit((data_0, data_t)) # original
...
vschnet, val_scores = train_with_sampler(1, learning_rate=5e-4)  # or the easier version, train(...)
...
# a list containing three trajectories, each contains fuzzy membership of each point into 6 states [np.ndarray (250000, 6)]
'''
# prep data for vampschnet
transformed_trajs = list()
for X in coordinates:
    traj = list()
    for datapoint in X:
        z = torch.LongTensor([0, 0, 1, 2, 0, 0, 0, 1, 2, 0])
        traj.append((z, datapoint))
    traj = np.array(traj)
    print("shape of traj : ", traj.shape)
    print("type of data (in script): ", type(traj[0][1]))
    print("shape of data (in script): ", traj[0][1].shape)
    transformed_trajs.append(vschnet.transform(traj))
    del X, traj   
#z = z.to(device=device, non_blocking=True, dtype=torch.long)
'''    
transformed_trajs = [vschnet.transform(X) for X in coordinates]
#np.save('transformed_trajs.npy', np.array(transformed_trajs))
